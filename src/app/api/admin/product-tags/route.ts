...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(result.docs.map(toTag), {
      headers: { 'Cache-Control': 'private, no-store' },
    })
  } catch (e) {
    console.error('[admin/product-tags GET]', e)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(toTag(created), { status: 201 })
  } catch (e) {
    console.error('[admin/product-tags POST]', e)
